...
**Generated:** 2026-02-16 (refreshed 2026-06-02)
**Latest commit referenced:** 792807c
**Active branch:** main

> Note: `CLAUDE.md` is a symlink to this file — Claude Code, OpenCode, and other agents all read these instructions.

## OVERVIEW

Client-side SPA validating Veeam VBR environments against VDC Vault requirements. Parses Healthcheck JSON locally, runs pre-flight checks. React 19 + Vite 7.3 + TypeScript 5.9 + Tailwind 4.1 + shadcn 3.8. The only outbound network calls are opt-in Veeam sizing-calculator requests, proxied via a Cloudflare Pages Function.

## STATUS: MVP + SOBR + REPOSITORIES + SIZING + HISTORY

Full pipeline operational: JSON upload → normalize → validate → dashboard. Comprehensive test suite (57 test files, 1267 tests in `src/__tests__/`). 13 validation rules in `src/lib/validator.ts` (8 core + 4 SOBR + 1 active-full). Motion/animation system with `prefers-reduced-motion` support. Vault sizing calculator with Veeam API integration (gated behind a privacy-consent dialog), multi-year growth projection, and headroom buffer. Repositories tab with size and immutability charts. Recent-scans history persisted in IndexedDB; global settings (target cloud, GFS/retention caps, buffer) persisted in localStorage. SOBR analysis covers capacity tier encryption, immutability, archive tier edition, and capacity tier residency.
...
│   ├── publish.yml               # Cloudflare Pages publish workflow
│   └── release.yml               # Release Please auto-versioning
├── functions/
│   └── api/
│       └── veeam-proxy.ts        # Cloudflare Pages Function — proxies POST to calculator.veeam.com
└── src/
    ...
    │   ├── settings.ts           # GlobalSettings, TargetCloud, DEFAULT_SETTINGS
    │   └── veeam-api.ts          # VmAgentRequest/Response shapes for the sizing API
    ├── lib/                      # → see src/lib/AGENTS.md (23 modules)
    ├── hooks/
    │   ├── use-analysis.ts       # State machine hook w/ race condition guard + step progression
    │   ├── use-calculator-api.ts # Sizing API flow: request build → call → derive → growth series
    │   ├── use-recent-scans.ts   # Recent-scans list backed by IndexedDB
    │   └── use-settings.ts       # Global settings + localStorage persistence
    ├── components/
    │   ├── ui/                   # shadcn primitives (card, button, badge, sheet, dialog, etc.)
    │   └── dashboard/            # 29 components → see src/components/dashboard/AGENTS.md
    └── __tests__/                # 57 test files, 1267 tests → see src/__tests__/AGENTS.md
```
...
| Validation rules   | PRD.md §4 + src/lib/validator.ts                              | 13 rules: 8 core (VBR version, config-backup encryption, job encryption, AWS, 2 agent rules, license, retention) + 4 SOBR + 1 active-full             |
...
| Domain types       | src/types/                                                    | healthcheck.ts (raw), domain.ts (normalized), validation.ts (results), calculator.ts (sizing), enriched-job.ts (join), settings.ts, veeam-api.ts      |
...
| Dashboard UI       | src/components/dashboard/AGENTS.md                            | 29 components, grouped by shell plus the overview / jobs / sizing / repositories tabs — see that file for the full map                                |
| Sizing calculator  | src/lib/calculator-aggregator.ts                              | Aggregates source TB, change rates, retention, GFS from job data                                                                                      |
| Veeam sizing API   | src/lib/veeam-api.ts + functions/api/veeam-proxy.ts           | Client calls `/api/veeam-proxy`; the Cloudflare Pages Function forwards to `calculator.veeam.com/vse/api/VmAgent`. Gated by calculator-consent-dialog |
| Repository charts  | src/lib/repo-aggregator.ts + chart-selectors.ts               | Aggregations for repositories tab and recharts visualizations                                                                                         |
| Sizing derivation  | src/lib/sizing-derivation.ts + sizing-buffer.ts               | Interprets the API response into GFS/immutability/buffer composition; applies headroom buffer                                                         |
| Growth projection  | src/lib/growth-projector.ts + growth-chart.tsx                | Async multi-year projection (one API call per year); rendered as a bar chart                                                                          |
| Global settings    | src/types/settings.ts + use-settings.ts + settings-dialog.tsx | Target cloud, GFS/retention caps, buffer; persisted to localStorage                                                                                   |
| Recent scans       | src/lib/indexed-db.ts + use-recent-scans.ts                   | IndexedDB persistence (FIFO-capped at 5); surfaced on the upload screen via file-upload.tsx                                                           |
| Validation helpers | src/lib/validation-selectors.ts                               | getBlockerValidations(), getPassingValidations(), getNoteValidations(), hasBlockers(), getBlockerCount()                                              |
| Version comparison | src/lib/version-compare.ts                                    | Parses and compares VBR version strings (e.g. "12.1.0.123")                                                                                           |
| Constants          | src/lib/constants.ts                                          | Shared constants (thresholds, defaults)                                                                                                               |
| Job detail sheet   | src/components/dashboard/job-detail-sheet.tsx                 | Right-side Sheet: Storage, Protection, Config, Session sections for EnrichedJob                                                                       |
| Job enrichment     | src/lib/enrich-jobs.ts                                        | enrichJobs() joins SafeJob[] with SafeJobSession[] via Map lookup                                                                                     |
| Formatting         | src/lib/format-utils.ts                                       | formatSize, formatPercent, formatDuration, formatTB, formatRatio, formatGFS                                                                           |
...
| `axios`, external `fetch`     | No backend calls — except via `src/lib/veeam-api.ts` → `/api/veeam-proxy` (consent-gated) |
...
| **Outbound calls**    | Only `/api/veeam-proxy` (sizing calculator), and only after explicit user consent via `calculator-consent-dialog`. No other `fetch`/`axios` permitted from app code. |
...
- **publish.yml**: Cloudflare Pages publish workflow (deploys `dist/` + `functions/` to Pages)
...
| UI work         | `frontend-design`, `web-design-guidelines`        |
| Completing work | `verification-before-completion`                  |
| Code review     | `code-reviewer`, `receiving-code-review`          |

> Skill names above must match the registered skill list of the agent in use. If a skill is missing or renamed, fall back to the closest available equivalent rather than fabricating one.
...
1. ~~MVP pipeline + UI~~ ✅ Done
2. ~~Motion/animation system~~ ✅ Done (checklist-loader, success-celebration, stagger, reduced-motion)
3. ~~Vault sizing calculator~~ ✅ Done (calculator-aggregator, calculator-inputs)
4. ~~Repositories tab + charts~~ ✅ Done (repo-aggregator, repositories-table, size/immutability charts)
5. ~~Cloudflare Pages deployment~~ ✅ Done (publish.yml + functions/api/veeam-proxy.ts)
6. ~~Veeam sizing API integration~~ ✅ Done (consent dialog, sizing-results, derivation + headroom buffer)
7. ~~Growth projection~~ ✅ Done (growth-projector, growth-chart — multi-year)
8. ~~Recent-scans history + global settings~~ ✅ Done (IndexedDB persistence, localStorage settings)
9. Polish: loading states, error recovery UX, mobile responsiveness
