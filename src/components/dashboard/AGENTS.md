...
6 components composing the results view. Motion system with stagger animations and `prefers-reduced-motion` support.
...
├── dashboard-view.tsx       # Main layout: header, summary cards, tabs (overview/jobs)
├── blockers-list.tsx        # Fail/warning alerts with severity ordering + stagger entrance
├── job-table.tsx            # TanStack Table: search, sort, paginate SafeJob[]
...
- **Tabs**: `dashboard-view.tsx` manages tab state. Overview shows BlockersList OR SuccessCelebration based on `hasBlockers`
